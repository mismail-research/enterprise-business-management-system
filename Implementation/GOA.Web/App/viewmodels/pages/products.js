...
define(['services/unitofwork', 'services/errorhandler', 'services/logger', 'services/utils', 'plugins/router'], function (unitofwork, errorhandler, logger, utils, router) {
    ...
    var vm = {
        ...
        activate: function () {
            alert(vm.arrivalDate());
            if (id == '') {
                unitofwork.rollback();
                var product = unitofwork.product.create();
                var buyer = unitofwork.buyer.create();
                var address = unitofwork.address.create();
                //alert(product.Buyers);
                //product.Buyers.push(buyer);
                //product.address.push(address);
                vm.product(product);

            }
            else {
                //unitofwork.product.findPerson(breeze.Predicate.create("contactId", "equals", id)).then(function (data) {
                //    vm.product(data[0]);
            ...
            //unitofwork.contacts.find(breeze.Predicate.create("contactType", "contains", "Company"), 1, 10
            //  ).then(function (data) {
            //      vm.companyies(data);
            //  });
            ga('send', 'pageview', { 'page': window.location.href, 'title': document.title });
            var self = this;
            unitofwork.product.all().then(function (data) {
                self.product(data)
            });
        ...
        close: function () {
            logger.logError("You Pressed Close Button.", null, null, true);
        },
